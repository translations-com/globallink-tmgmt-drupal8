 ...
 * )
 */
class GlobalLinkTranslator extends TranslatorPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function requestTranslation(JobInterface $job) {
    ...
    try {
      ...
      $due_datetime = $due_setting['object'];
      ...
      $data = $this->formatManager->createInstance('xlf')->export($job);
      $document_settings = [
        'name' => $job->label(),
      ...
      foreach ($job->getItems() as $job_item) {
        $job_item->addRemoteMapping(NULL, $submission_result_id, ['remote_identifier_2' => $upload_result_id]);
      }

      $job->submitted('Job has been successfully submitted for translation.');
      $job->addMessage('Ticket with id %submission_id created', ['%submission_id' => $submission_result_id]);
    }
    catch (\Exception $e) {
      ...
      $job->addMessage('Job has been rejected with following error: @error',
        ['@error' => $e->getMessage()], 'error');
    }
  }
   ...
   */
  public function retrieveTranslation($ticket_id, JobInterface $job, $add_message = TRUE) {
  ...
  public function fetchJobs(Job $job) {
    ...
    foreach ($completed_translations as $completed_translation) {
      if (isset($completed_translation->clientIdentifier)) {
        $completed_by_uuid[$completed_translation->clientIdentifier] = $completed_translation;
      }
    }

    if (isset($completed_by_uuid[$job->uuid()])) {
      $translation = $completed_by_uuid[$job->uuid()];
      $this->retrieveTranslation($translation->ticket, $job);
    }
    else {
